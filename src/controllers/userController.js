...
class UserController {
    static async createUser(req, res) {
        const { email, password, artist = false } = req.body;
        ...
        try {
            ...
            const user = new User({ email, password, artist });
            await user.save();

            res.status(201).json({ message: "User created successfully.", user: { email: user.email, artist: user.artist } });
        } catch (error) {
